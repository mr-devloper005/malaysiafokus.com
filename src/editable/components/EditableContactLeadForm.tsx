...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6">
      ...
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-2xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]' : 'bg-red-950/40 text-red-300'}`}>
      ...
      <button
        ...
        className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-6 text-sm font-semibold text-[var(--slot4-on-accent)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-70"
      >
