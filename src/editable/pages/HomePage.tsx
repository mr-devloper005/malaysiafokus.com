...
import { EditableCategoryGrid, EditableHomeCta, EditableHomeHero, EditableHowWeWork, EditableMagazineSplit, EditableStoryRail, EditableTimeCollections } from '@/editable/sections/HomeSections'
...
export default async function HomePage() {
  // Profiles are never surfaced publicly, so the home experience centers on the
  // first non-profile enabled task (collections / sbm) rather than just the
  // first enabled task — otherwise every home link would point at /profile.
  const primaryTask = (
    SITE_CONFIG.tasks.find((task) => task.enabled && task.key !== 'profile')?.key ||
    SITE_CONFIG.tasks.find((task) => task.enabled)?.key ||
    'article'
  ) as TaskKey
  ...
  return (
    <EditableSiteShell>
      ...
      <EditableCategoryGrid primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} timeSections={timeSections} />
      <EditableHowWeWork />
